...
public class ProducerTest {
	...
	private List<Person> getPersons() {
		...
		List<Person> list = new ArrayList<>();
  		...
  		try (Stream<String> stream = Files.lines(path)) {
  			stream.forEach((s) ->
  				list.add(new Person(id.incrementAndGet(), s))
  			);
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	...
	private Consumer<Long, Person> createConsumer() {
		ConsumerFactory<Long, Person> cf = new DefaultKafkaConsumerFactory<>(createConsumerProps(), new LongDeserializer(), new JsonDeserializer<>(Person.class));
